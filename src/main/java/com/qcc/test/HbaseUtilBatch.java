...
import com.qcc.baseinfo.entity.CompanyInvest;
...
//				conf.set("hbase.zookeeper.quorum", "node02,node03,node04");
    ...
    /**
     * 限制批量查询遍历hbase的数据company_invest
     * @param rowkeyList
     * @return
     * @throws IOException
     */
    public Result[] qurryHbaseTableInvestLimitBatch(String tableName, List<String> rowkeyList) throws IOException {
        Result[] results;
        List<Result> resultList = new ArrayList<>();
        Table table = connection.getTable(TableName.valueOf(tableName));// 获取表
        if (rowkeyList.size() > Constants.QUERY_HBASE_LIMIT) {
            while (true) {
                List<Get> getList = new ArrayList();
                List<String> curRowList = new ArrayList<>();
                curRowList.addAll(rowkeyList);
                if (rowkeyList.size() > Constants.QUERY_HBASE_LIMIT) {
                    for (int i = Constants.ZERO; i < Constants.QUERY_HBASE_LIMIT; i++) {
                    ...
                    if (curResults.length > Constants.ZERO) resultList.addAll(Arrays.asList(curResults));
                } else if (rowkeyList.size() > Constants.ZERO) {
                    for (int i = Constants.ZERO; i < rowkeyList.size(); i++) {
            ...
            return table.get(getList);//重点在这，直接查getList<Get>
        }
    ...
    public List<String> queryHbaseInvestGetKeyNoRecursion(List<String> keyNos, List<String> resultKeyNos, int level) {
        ...
        if (CollectionUtils.isNotEmpty(keyNos)) {
            try {
                resultsInvest = qurryHbaseTableInvestLimitBatch(Constants.HBASE_TBALE_NAME_COMPANY_INVEST, keyNos);
            } catch (IOException e) {
                e.printStackTrace();
    ...
    private List<CompanyInvestmentData> queryHbaseModifierEncapsulateDataByList(List<CompanyInvestmentData> companyInvestmentDatas,
        ...
        try {
            oldResults = qurryHbaseTableInvestLimitBatch(Constants.HBASE_TBALE_NAME_COMPANY_INVEST, keyNOs);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public List<String> queryHbaseMongoCompanyGetKeyNoRecursion(List<String> keyNos, List<String> resultKeyNos, int level) {
        ...
        if (CollectionUtils.isNotEmpty(keyNos)) {
            try {
                resultsInvest = qurryHbaseTableInvestLimitBatch(Constants.HBASE_TBALE_NAME_MONGO_COMPANY, keyNos);
            } catch (IOException e) {
                e.printStackTrace();
                logger.error("查询hbase错误keyNos=" + keyNos + "tableName" + Constants.HBASE_TBALE_NAME_MONGO_COMPANY);
            }
